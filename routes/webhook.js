...
async function addTags(contactId, classification, locationId) {
  try {
    ...
    // Get dynamic data from pricing service
    const PricingService = require('../services/pricingService');
    const allBrands = PricingService.allBrands || new Set();
    const allParts = PricingService.allParts || new Set();
    
    // Dynamic brand mapping - create from discovered brands
    const dynamicBrandMapping = {};
    for (const brand of allBrands) {
      // Capitalize first letter for tag
      const capitalizedBrand = brand.charAt(0).toUpperCase() + brand.slice(1);
      dynamicBrandMapping[brand] = capitalizedBrand;
      
      // Add variations
      if (brand === 'apple') dynamicBrandMapping['iphone'] = 'Apple';
      if (brand === 'samsung') dynamicBrandMapping['galaxy'] = 'Samsung';
      if (brand === 'xiaomi') dynamicBrandMapping['redmi'] = 'Xiaomi';
    }
    
    // Dynamic service mapping - create from discovered parts
    const dynamicServiceMapping = {};
    for (const part of allParts) {
      // Capitalize first letter for tag
      const capitalizedPart = part.charAt(0).toUpperCase() + part.slice(1);
      dynamicServiceMapping[part] = capitalizedPart;
    }
    
    // Add common English-to-Spanish variations (CRITICAL for English inputs)
    dynamicServiceMapping['screen'] = 'Pantalla';
    dynamicServiceMapping['display'] = 'Pantalla';
    dynamicServiceMapping['battery'] = 'Bateria';
    dynamicServiceMapping['camera'] = 'Camara';
    dynamicServiceMapping['speaker'] = 'Altavoz';
    dynamicServiceMapping['microphone'] = 'Microfono';
    dynamicServiceMapping['cover'] = 'Tapa';
    dynamicServiceMapping['back'] = 'Tapa';
    dynamicServiceMapping['button'] = 'Boton';
    dynamicServiceMapping['flex'] = 'Flex';
    dynamicServiceMapping['antenna'] = 'Antena';
    
    console.log(`🔍 Using dynamic mappings: ${Object.keys(dynamicBrandMapping).length} brands, ${Object.keys(dynamicServiceMapping).length} parts`);
    ...
    if (classification.device_brand && classification.device_brand !== 'unknown') {
      ...
      const predefinedBrand = dynamicBrandMapping[normalizedBrand];
      if (predefinedBrand) {
        tags.push(predefinedBrand);
      }
    }
    
    // ALSO check device_model for brand extraction (fallback)
    if (classification.device_model && classification.device_model !== 'unknown') {
      const modelLower = classification.device_model.toLowerCase();
      for (const [key, value] of Object.entries(dynamicBrandMapping)) {
    ...
    if (classification.service_type && classification.service_type !== 'consulta general' && classification.service_type !== 'general' && classification.service_type !== 'saludo') {
      const normalizedService = classification.service_type.toLowerCase();
      const predefinedService = dynamicServiceMapping[normalizedService];
    ...
    if (uniqueTags.length === 0) {
      console.log('🏷️ No relevant tags to add');
      return;
    }
    
    console.log('🏷️ Adding dynamic tags:', uniqueTags);
...
router.get('/health', async (req, res) => {
  try {
    ...
    await SimplifiedPricingService.getPricingData();
    ...
    res.json({
      ...
      features: {
        ...
        universal_part_detection: 'enabled',
        watch_support: 'enabled',
        cross_brand_suggestions: 'enabled',
        dynamic_tagging: 'enabled'
      }
    });
  } catch (error) {
    res.status(500).json({
...
router.post('/search', async (req, res) => {
  try {
    ...
    const products = await SimplifiedPricingService.searchProducts(query, 10);
    ...
    res.json({
      ...
      products: products.map(p => ({
        name: p.Producto || Object.values(p)[0],
